...
Como os dashboards são compostos exclusivamente por arquivos estáticos, foram publicados gratuitamente na plataforma Netlify em dois ambientes complementares. O primeiro reúne os dados de Mossoró-RN e as páginas de diagnóstico dos ODS 9 e 16, e corresponde ao ambiente utilizado na avaliação de aceitação tecnológica descrita no Capítulo~\ref{cap:validacao-tam}, disponível em \url{https://dashboard-ods-mossoro.netlify.app}. O segundo corresponde à versão aprimorada e genérica da ferramenta, capaz de carregar arquivos JSON de análise de qualquer localidade e de gerar novas análises a partir do formulário de configuração descrito na Seção~\ref{sec:formulario-config}, disponível em \url{https://dashboard-ods.netlify.app}. Essa estratégia transformou a etapa de visualização em uma camada de apresentação leve, portátil e de fácil publicação, que também pode ser executada localmente em navegador, ampliando a acessibilidade da ferramenta para gestores públicos, pesquisadores e demais interessados.
