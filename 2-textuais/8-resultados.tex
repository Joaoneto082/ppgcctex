...
A baixa proporção de avaliações positivas (14,6\%) e o fato de que 56,9\% das análises contribuem diretamente para o baixo desempenho reforçam a pontuação muito baixa (0--39,99) atribuída a Mossoró no ODS 9 pelo IDSC \cite{idsc2024}. Os scores de confiança elevados (média de 0,87) indicam que o modelo identificou alguns aspectos problemáticos nos textos analisados.
...
A amostra de 12 estabelecimentos reflete a realidade institucional do município: Mossoró possui um número limitado de órgãos públicos de justiça e segurança formalmente cadastrados na Google Places API, e o sistema capturou os principais estabelecimentos disponíveis. Essa cobertura dos órgãos institucionais possibilitou uma percepções sobre o ODS 16 na cidade, ainda que o número absoluto de locais seja menor do que o do ODS 9, que inclui também estabelecimentos comerciais e de infraestrutura.
...
Os trechos ilustram duas características relevantes da geração automatizada: a fidelidade aos números agregados da coleta, reproduzidos corretamente nos textos, e a incorporação de evidências textuais extraídas diretamente das avaliações dos cidadãos, como a descrição da rodoviária. A consistência entre os achados automatizados e as evidências conhecidas podem servir de confirmação da capacidade do sistema de produzir diagnósticos confiáveis a partir de dados não estruturados.
...
Como falado anteriormente, embora o estudo de caso principal desta dissertação concentre-se em Mossoró-RN, com ênfase nos ODS 9 e 16, a implementação também foi avaliada quanto à sua capacidade de generalização para outros objetivos e municípios. Essa verificação foi realizada por meio da execução do motor de análise em diferentes combinações de ODS, cidade e conjunto de dimensões, mantendo a mesma lógica de coleta, filtragem, análise por aspecto, geração de JSON e produção de diagnóstico automatizado.

Convém reforçar que apenas os ODS 9 e 16 em Mossoró-RN foram efetivamente validados; todos os demais cenários relatados nesta seção têm finalidade exclusiva de demonstrar a capacidade de generalização da abordagem, e não devem ser lidos como diagnósticos consolidados.

A generalização foi testada em dois níveis. No primeiro, de natureza geográfica, os modelos dos ODS 9 e 16 foram reaplicados em outras cidades (Natal-RN, Fortaleza-CE, João Pessoa-PB e Salvador-BA), mantendo a estrutura de análise definida para cada objetivo e alterando apenas a localidade. No segundo, de natureza temática, a versão genérica orientada por arquivos de configuração JSON foi executada sem alteração do código-fonte para os ODS 3 (Saúde e Bem-estar) e 4 (Educação de Qualidade) -- objetivos escolhidos apenas para exercitar a flexibilidade do motor, cujos arquivos de configuração foram produzidos com o formulário interativo de geração de JSON. Os detalhes operacionais dessas execuções, incluindo os arquivos de configuração utilizados, os volumes coletados e as devidas ressalvas de escopo, são apresentados nas Seções~\ref{sec:generalizacao-multiplos-ods} e~\ref{sec:generalizacao-geografica}.
