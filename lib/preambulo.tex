...
\usepackage{placeins}                               % \FloatBarrier (evita floats após seções)
\usepackage{amsgen}
...
\usepackage[font=singlespacing,format=plain,justification=centering,skip=0pt,singlelinecheck=true]{caption}            % Altera o comportamento da tag caption (títulos centralizados)

% Centraliza também as notas de fonte (\legend)
\let\oldlegend\legend
\renewcommand{\legend}[1]{%
  {\captionsetup{justification=centering,singlelinecheck=true}\oldlegend{#1}}%
}
...
\usepackage{float}            % [H] nas figuras — deve vir depois de ppgcctex (evita conflito com \newfloat[chapter]{quadro})
...
% Configuração do pacote listings para exibição de código-fonte
